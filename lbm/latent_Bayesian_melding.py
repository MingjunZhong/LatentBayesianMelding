...
class LatentBayesianMelding(object):
    ...
    ----------
    model :
        This is the latent Bayesian melding model for adding constrains to
        the baseline fhmm model. The problem is relaxed to a SOCP problem,
        which is then solved by using MOSEK solver.
        ...
        with Model("composite model") as M:
            ...
            for i, appliance in enumerate(self.individual):
                ...
                c_relaxStateSumColumn = M.constraint( 
                        Expr.sub( Expr.mul(vtemp, 1.0*np.ones((nosOfStates,1))),
                        Variable.reshape(stateVariableOfAppliance[appliance].
